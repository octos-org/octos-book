...
生产化相关的代码原本全部住在 `octos-cli` 里:admin token 存储是 `crates/octos-store/src/admin_token_store.rs`,setup 向导状态是 `crates/octos-store/src/setup_state_store.rs`,SMTP 密码是 `crates/octos-store/src/smtp_secret_store.rs`,租户管理是 `crates/octos-services/src/tenant.rs`,自更新是 `crates/octos-services/src/updater.rs`。它们都是叶子模块,不依赖 CLI 的命令层,却和几千行的命令代码一起编译、一起测试。
...
拆分前后的形状变化可以用三个切面说清。第一是依赖边方向:抽取后两个 crate 在 Cargo 依赖图上只指向 octos-core(services 再加 octos-llm、octos-bus),不再有任何边指回 octos-cli;抽取前这些模块与命令层之间没有隔离,`octos serve` 改一行 AppState 定义,八个 store 全部重编。第二是测试输入:拆前测 `AdminTokenStore` 要在 octos-cli 的测试树里搭 tempdir 夹具;拆后 crate 内自带 `#[cfg(test)]`,crates/octos-store/src/admin_token_store.rs 文件尾部的测试直接断言 `0o600` 权限位(`crates/octos-store/src/admin_token_store.rs:182`),不依赖任何外部环境。第三是编译产物粒度:CLI 二进制之外的工具(诊断、报表、未来的 CLI-agent 适配)现在可以只链 store/services,不必拖进整个 CLI。octos-diagnostics 的抽取动机更直接:octoscode 的 doctor/install_method 模块与 octos 服务端要做同一套探测,复制两份必然漂移,于是把「产品无关」部分下沉,产品差异收窄到 `ProductSpec` 一个入口(ADR 见 crate 文档,octoscode#182)。

### 15.1.2 octos-store:持久化层,9 文件 2,664 行

抽取后的 `crates/octos-store/` 是一个纯持久化 crate,9 个源文件共 2,664 行(`crates/octos-store/src/lib.rs` 18 行 + 8 个 store 模块,行数来自事实表对 main @ 9c157101 的统计)。`crates/octos-store/src/lib.rs` 的模块文档直接说明了出身:"Self-contained persistence / state stores extracted from `octos-cli`"。

| 文件 | 行数 | 职责 | 关键符号 |
|---|---|---|---|
| `crates/octos-store/src/admin_token_store.rs` | 184 | admin token 的 salt+hash 存储 | `AdminTokenRecord`(:14) `AdminTokenStore`(:56) |
| `crates/octos-store/src/admin_audit_store.rs` | 270 | admin 操作审计(redb) | `AdminAuditStore`(:109) |
| `crates/octos-store/src/approvals_audit.rs` | 414 | 审批决策审计(JSON-Lines 追加) | `ApprovalsAuditLog`(:119) |
| `crates/octos-store/src/login_allowlist.rs` | 258 | 登录白名单(带锁重试) | `LoginAllowlistStore`(:28) |
| `crates/octos-store/src/setup_state_store.rs` | 151 | 首启向导状态 | `SetupStateStore`(:24) |
| `crates/octos-store/src/smtp_secret_store.rs` | 133 | SMTP 密码文件 | `SmtpSecretStore`(:17) |
| `crates/octos-store/src/usage_ledger.rs` | 901 | 用量台账(redb) | `UsageEvent`(:42) `PersistentUsageLedger`(:323) |
| `crates/octos-store/src/user_store.rs` | 335 | 多用户管理(每用户一 JSON) | `UserStore`(:40) |
...
以 `crates/octos-store/src/admin_token_store.rs` 为例看抽取后的形态。文件首页文档写明:"Hashed admin auth token stored at `{data_dir}/admin_token.json`. Replaces the static config/env bootstrap token once rotated." 记录结构只有四个字段(`crates/octos-store/src/admin_token_store.rs:14-21`):
...
规模最大的 `crates/octos-store/src/usage_ledger.rs`(901 行)是给「每个 LLM run 记一行」的台账,`UsageEvent` 结构在 :42,redb 持久化入口 `PersistentUsageLedger` 在 :323。字段设计有一个值得留意的点:cache 读取 token 被单独记录,文档注释明确说明 provider 报告的 prompt 总数里含缓存命中,而 `TokenUsage` 在 provider 边界就把缓存部分减掉了,所以完整 prompt 数是 `input_tokens + cache_read_tokens`(`crates/octos-store/src/usage_ledger.rs:52-66`)。这让「缓存到底省了多少钱」成为事后可查的数据,而不必现场探测 API。

### 15.1.3 octos-services:支撑服务层,8 文件 3,223 行

第二个抽出的是 `crates/octos-services/`,8 个源文件共 3,223 行。`crates/octos-store/src/lib.rs` 文档同样点明出身:"Self-contained support services extracted from `octos-cli`"。模块共 7 个:

| 文件 | 行数 | 职责 | 关键符号 |
|---|---|---|---|
| `crates/octos-services/src/cli_agent_adapter.rs` | 587 | 未来 CLI-agent 适配的进程边界 | `CliAgentProcess`(:145) |
| `crates/octos-agent/src/agent/compaction.rs` | 320 | 会话压缩 | `maybe_compact`(:37) |
| `crates/octos-services/src/config_context.rs` | 698 | 配置/凭据/数据路径的唯一解析入口 | `resolve_config_context`(:151) |
| `crates/octos-services/src/persona_service.rs` | 479 | 定期生成沟通风格画像 | `PersonaService`(:97) |
| `crates/octos-services/src/soul_service.rs` | 139 | 每用户 personality 存储 | `read_soul`(:24) |
| `crates/octos-services/src/tenant.rs` | 512 | 租户隧道管理 | `TenantStore`(:82) `render_frpc_config`(:255) |
| `crates/octos-services/src/updater.rs` | 473 | 自更新:下载、校验、备份、替换、回滚 | `Updater`(:33) |
...
`crates/octos-services/src/config_context.rs`(698 行)是这批模块里最值得停下来的一个。文档自称 "Canonical config/auth/data path resolver — the single source of truth":所有配置路径、凭据路径、数据目录路径必须经 `resolve_config_context` 解析,不允许各模块自行拼接。这条规则直接回应了旧版的一个痛点:同一路径在多个模块各拼一遍,运行模式一多就漂移。
...
这个 crate 的模块文档里有一个反复强调的词:product-agnostic。它不为 octos 专有,同一个诊断内核要同时服务 octos 与 octoscode 两个二进制。实现这条复用的接缝是 `crates/octos-diagnostics/src/spec.rs` 里的 `ProductSpec`(:173 行文件,事实表 §3):
...
文件清单:crates/octos-diagnostics/src/checks.rs 400 行(终端环境、config/data 目录等本地检查)、crates/octos-diagnostics/src/github.rs 321 行(blocking GitHub Releases client)、crates/octos-diagnostics/src/install_method.rs 505 行(安装方式探测)、crates/octos-diagnostics/src/locate.rs 278 行(PATH 解析与 shadow 检测)、crates/octos-diagnostics/src/report.rs 312 行(报告模型,`CheckStatus`/`Check`)、crates/octos-diagnostics/src/spec.rs 173 行(ProductSpec 接缝)、crates/octos-diagnostics/src/update.rs 198 行(semver 解析与纯函数更新计划器)。
...
octos 的多租户不是一个 tenant ID 字段,而是 Profile、Account、User、Session 四层叠出来的隔离模型。代码侧的 tenancy 词汇分布很能说明问题:全词 `tenant` 在 crates/ 下命中 2,261 次,其中最多的不是任何 tenant 模块,而是 `crates/octos-cli/src/autonomy/agent_orchestrator.rs`(921 次),其次才是 crates/octos-cli/src/api/admin.rs(138)、crates/octos-cli/src/api/auth_handlers.rs(130)与 crates/octos-cli/src/api/ui_protocol_transport.rs(102)(事实表 §4 的 grep 统计)。租户概念已经渗透进编排、管理 API、认证与传输各层。
...
租户之下还有子账号层。创建入口是 `create_sub_account`(`crates/octos-cli/src/profiles.rs:2048`),规则有三条:父 profile 必须存在;父自己不能已是子账号(禁止多层嵌套);数量受 `MAX_SUB_ACCOUNTS_PER_PARENT = 10` 上限约束(`crates/octos-cli/src/profiles.rs:16`)。子账号 ID 形如 `{parent_id}--{sub}`,创建时 `config.llm` 置 None(`crates/octos-cli/src/profiles.rs:2092` 附近),等运行时解析继承。

继承发生在 `resolve_effective_profile`(`crates/octos-cli/src/profiles.rs:2403-2450`),规则按字段分三档:
...
有一项明确不继承:customer-installed skills。子账号的 skills 目录严格按当前 account 解析,不随 `resolve_effective_profile` 的 config 继承链传递(事实表 §4 引 `crates/octos-cli/src/profiles.rs:360-407` 的 skills layer 语义)。安全含义直白:父账号装了一个 skill,不代表每个子账号都自动获得执行它的能力。
...
租户 ID 的校验也值得一看:测试用例覆盖了拒绝空 ID、拒绝大写、拒绝首尾连字符、拒绝路径穿越(`../etc`、`..`、`foo/bar`、`.hidden` 全部拒绝,`crates/octos-services/src/tenant.rs:270-310` 附近的测试组)。因为租户 ID 会直接拼进 profile 目录路径与文件名,ID 校验就是路径安全的第一道闸。
...
诊断路径独立于更新:`octos doctor` 走 octos-diagnostics 的检查集(终端环境、config/data、PATH/shadow、网络),按 `[✓]/[!]/[✗]` 产出报告,支持 JSON support-bundle 输出;`update --check` 走 Stage 2 的 GitHub client 与纯函数计划器(crates/octos-diagnostics/src/update.rs 的 `plan`)比对 semver,只报告不执行。两条路径合起来覆盖了「升级前确认环境」与「升级本身」两步。
...
- **frp 项目**:https://github.com/fatedier/frp — crates/octos-services/src/tenant.rs 隧道方案依赖的底层转发工具
- **redb**:https://github.com/cberner/redb — octos-store 用于 usage ledger 与 admin audit 的嵌入式 KV 存储
- **Prometheus 文本格式**:https://prometheus.io/docs/instrumenting/exposition_formats/ — crates/octos-cli/src/api/metrics.rs 渲染与 OperatorSummary 解析的线协议
