...
> Ch5-6 回答「我发的消息到底经历了什么」，Ch14 讲清运行模式与配置后，直接进入双环部分：octoscode 客户端（Ch19）、外环协议 OLP v2（Ch20）、herdr 运维实务（Ch21）。Ch18 的 goal/peer 是 Ch20 的服务端前提，时间允许先读。跳过实现细节侧栏不影响理解。
...
愿这本书帮你建立的不只是对 octos 的理解，还有一套评估 Agent 基础设施的尺子。读任何 Agent 系统时，带着四问：

- 安全边界在哪：哪个进程、哪条系统调用、哪行代码在强制？
- 谁在强制协议：编译器、运行时、契约测试，还是仅有一份文档？
- 状态能否恢复：崩溃之后，重启到哪个断点，丢掉哪段工作？
- 错误去了哪里：被吞掉、被记录、被上报，还是被静默重试？
