...
`score()` deliberately uses `throughput` rather than raw latency as the runtime speed signal, because single-request latency tracks task complexity too closely to serve as a direct quality signal across providers.
