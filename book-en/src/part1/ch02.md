...
The Rust ecosystem has two mainstream error-handling libraries, `anyhow` and `eyre`. Both provide type-erased error reporting (`anyhow::Error` / `eyre::Report`), and octos picked `eyre`/`color-eyre`. The choice deserves a close look.
