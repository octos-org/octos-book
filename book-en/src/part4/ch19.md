...
Start with the conclusion. In protocol mode, octoscode is a dumb client. The Scope section of `octoscode/docs/ARCHITECTURE.md:3` draws the boundary in one sentence:
...
One architectural constraint deserves a name. The store's two entries do not just mutate state; they produce follow-up commands, which makes the reducer both a state machine and a follow-up command constructor. The designers were well aware of the risk of 44,000 lines, and their answer was not to split the file but to converge all state transitions into a single point: any bypass that mutates `AppState` directly creates a second source of truth for replay and reconnect hydration. Chapter 18's server-side GoalLedger gathering state transitions into one impl block through 39 methods is the same discipline mirrored on the server: the entry to state transition must be unique, even if the impl block grows enormous as a result.
...
1. `approval_blocked` (state=blocked, priority=1100, region=whole_recent) at `herdr/src/detect/manifests/octoscode.toml:8`: keywords include `Approval | y once | s session | n deny`, `Waiting on you`, `Ctrl+R/Alt+A answer`, and the Chinese variant (shenpi, y for this time, s for this session);
