...
The boundary design is the heart of this surface: the server exposes exactly one session-level tool, `run_octos_session` (constant `RUN_OCTOS_SESSION_TOOL`, `crates/octos-agent/src/mcp_server.rs:66`). Each call runs one complete octos session (load profile, construct the LLM, create the agent, execute the prompt, validate artifacts), and what the outer loop receives is the aggregated result: it sees no internal tool calls, iteration events, or progress stream. octos's internal tool catalog (59 tools; see Chapter 6) is not turned outward. This is the trade of a coarse-grained task boundary against a fine-grained tool proxy: the outer orchestrator decomposes the task; the octos session executes autonomously inside the boundary.
...
4. mcp-serve exposes only `run_octos_session`: a session-level aggregated result, with the internal tool catalog never turned outward; stdio is parent-trust, and HTTP must carry a token.
