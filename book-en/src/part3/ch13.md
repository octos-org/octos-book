...
The handler-name-to-string mapping grew to 9 entries in step, including `shell_check`, `notify`, `wait` (`crates/octos-pipeline/src/executor.rs:251-259`).
...
`crates/octos-pipeline/src/human_gate.rs` does exist, providing abstractions such as `HumanInputProvider` and `ChannelInputProvider`, with a default timeout of 5 minutes (`DEFAULT_INPUT_TIMEOUT`, `crates/octos-pipeline/src/human_gate.rs:14-15`). But after checking the current source I can state this plainly: those abstractions are not wired into the `build_handlers()` or `execute_graph()` main path. So "Gate = human-approval node" is not an accurate description of the current implementation.
