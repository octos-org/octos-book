...
In the manual pattern, "the sub-agent got the task, so it runs"; the first requirement of formalizing the primitive is making "may it run" an explicit decision. Swarm has three gates, at different positions, over different objects, with different failure dispositions.
