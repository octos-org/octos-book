...
Depth-addition record: the first draft ran 4,565 Chinese characters; this revision expanded it to about 5,900. Five new passages: the relationship between the two coordinate systems of capability domains and the `TOOL_GROUPS` policy groups (the `group:delegated` superset invariant and its guard test at `crates/octos-agent/src/tools/policy.rs:187`), the design motive for per-domain file splitting (shared invariants within a domain, isolated threat models between domains), the sediment order of `ToolRegistry` fields and the cache contract of `specs()` ordering (`crates/octos-agent/src/tools/registry.rs:179`, `:746`), the parent/child directionality of `clear_spawn_only` and the `task_handle` envelope (`crates/octos-agent/src/tools/registry.rs:355`, `:388`), and the allocation-free design of `estimate_json_size` (`crates/octos-agent/src/tools/registry.rs:95-119`). All new line numbers were re-verified for this revision.
